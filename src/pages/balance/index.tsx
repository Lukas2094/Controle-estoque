...
const Balance = () => {
    ...
    const BuildBalanceArray = () => {
        ...
        const db_stock_output = localStorage.getItem("db_stock_outputs")
        ...
        db_products.map((prod) => {
            const entries = db_stock_entries.filter((item) => item.product_id === prod.id)
            .map((entry) => Number(entry.amount))
            .reduce((acc , cur) => acc + cur , 0);

            const outputs = db_stock_output.filter((item) => item.product_id === prod.id)
            .map((entry) => Number(entry.amount))
            .reduce((acc, cur) => acc + cur ,0);
    ...
    const handleFilterProducts = () => {
        ...
        const newArray = cmbProducts.filter((item) => item.product_id === productFiltered)
                ...
                <Box w={'100%'}>
                    <SimpleGrid minChildWidth={240} h={'fit-content'} spacing={'6'}>
                       ...
                       >
                        ...
                        {cmbProducts && cmbProducts.length > 0 && cmbProducts.map((item , i) => (
                            <option value={item.product_id} key={i}>{item.product_name}</option>
                        ))}
