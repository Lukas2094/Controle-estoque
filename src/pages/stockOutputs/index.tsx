...
const StockOuputs = () => {
    ...
    useEffect(() => {
        const db_stock_entries = localStorage.getItem("db_stock_output") 
        ? JSON.parse(localStorage.getItem("db_stock_output")) : [];
    ...
    const handlenewEntry = () => {
        ...
        if(listStockEntries && listStockEntries.length) {
            localStorage.setItem("db_stock_output" , JSON.stringify([...listStockEntries, {id , amount , product_id}]));
            setStockEntries([...listStockEntries, {id , amount , product_id}]);
        } else {
            localStorage.setItem("db_stock_output" , JSON.stringify([{id, amount , product_id}]));
    ...
    const removeEntries = (id) => {
        ...
        localStorage.setItem("db_stock_output" , JSON.stringify(newArray));
               ...
               <Box w={'100%'}>
                 <SimpleGrid minChildWidth={240} h={'fit-content'} spacing={'6'}>
                     ...
                     >
                        ...
                        {listProducts && listProducts.length > 0 && listProducts.map((item , i) => (
                    ...
                    <Table mt={'6'}>
                        <Thead>
                            <Tr>
                                ...
                                <Th fontWeight={'bold'} fontSize={'14px'}>Qtd.</Th>
                                <Th></Th>
                            </Tr>
                        </Thead>
                        ...
                        <Tbody>
                            {listStockEntries.map((item , i) => (
                                <Tr key={i}>
                                  <Td color={'gray.500'}>{getProductsById(item.product_id)}</Td>
